...
void TestParse()
{
	...
	JsonValue json = JsonParser::Parse(builder.Build().GetString());
	for (UINT i = 0, l = json.GetLength(); i < l; ++i)
	{
		JString name = json.GetName(i);
		JString type = json.Get(name)->GetType();
		JString value = json.Get(name)->GetString();
		Print("Name=[%-7s] Type=[%-6s] ValueOfString=\"%s\"\n", name.GetString(), type.GetString(), value.GetString());
	}

 	Print("%s\n", JsonFormater::Format(json, 4).GetString());
}
